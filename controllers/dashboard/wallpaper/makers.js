 ...
 * =======================================================================================================
 *
 * Seezoo dashboard wallpaper category Controller
 *
 * @package Seezoo Core
...
ClassExtend('Controller', function() {
	...
	this.ajaxEdit = function (ev) {
		...
		states.get(1).show('i');
		DOM(ev.target).hide();
		this.ajax.post('dashboard/wallpaper/makers/ajax_update_category/' + this.config.item('sz_token'), {
	...
	this.addCategoryMaster = function (ev) {
		var c = document.getElementsByName('maker_name_master')[0],
		...
		this.ajax.post('dashboard/wallpaper/makers/ajax_add_category/' + this.config.item('sz_token'), {
			...
			success : function (resp) {
				...
				li = DOM.create('li');
				if (ul.detect('li[class!=ttl]').length === 0 || !ul.detect('li[class!=ttl]').get(0).hasClass('odd')) {
					li.addClass('odd');
				}
				html = [
				            '<span>' + c.value + '</span>',
				            '<form class="init_hide" method="post" action="' + that.config.siteUrl() + 'wallpape/maker/ajax_category_edit">',
				            '<input type="text" value="' + c.value + '" name="maker_name" />',
				            '<input type="hidden" value="' + json.wall_maker_id + '" name="wall_maker_id" />',
							...
							'<a href="' + that.config.siteUrl() + 'dashboard/wallpaper/makers/ajax_delete_category/' + json.sz_blog_category_id + '" class="li_del">削除</a>',
							'</p>',
				            ];
				li.html(html.join('\n'))
					.event('mouseover', function (ev) { DOM(this).addClass('hover');})
					.event('mouseout', function (ev) { DOM(this).removeClass('hover');})
					.appendTo(ul.first(), 'after');
